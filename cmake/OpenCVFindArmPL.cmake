...
function(ocv_armpl_download)
  ...
  if(NOT EXISTS "${ARMPL_DOWNLOADED_FILE}")
    message(STATUS "ARM Performance Libraries: Downloading...")
    file(DOWNLOAD 
      "${ARMPL_DOWNLOAD_URL}" 
      "${ARMPL_DOWNLOADED_FILE}"
    ...
    if(NOT DOWNLOAD_STATUS_CODE EQUAL 0)
      message(WARNING "ARM Performance Libraries: Download failed - ${DOWNLOAD_ERROR_MSG}")
      return()
    endif()
  endif()
  ...
  if(WIN32 AND ARMPL_BINARIES_ARCHIVE MATCHES "\\.msi$")
    ...
    execute_process(
      COMMAND msiexec /a "${ARMPL_MSI_NATIVE_PATH}" /qn "TARGETDIR=${ARMPL_TARGET_NATIVE_PATH}"
    ...
    if(NOT ARMPL_H_CHECK)
      message(WARNING "ARM Performance Libraries: MSI extraction failed")
      message(STATUS "  MSI location: ${ARMPL_DOWNLOADED_FILE}")
      message(STATUS "  Extract manually to: ${ARMPL_EXTRACT_DIR}")
      message(STATUS "  Options: 7-Zip, lessmsi, or msiexec /a manually")
      return()
    endif()

    file(GLOB_RECURSE ARMPL_H_LOCATIONS "${ARMPL_EXTRACT_DIR}/**/armpl.h")
    list(GET ARMPL_H_LOCATIONS 0 FIRST_ARMPL_H)
    get_filename_component(ARMPL_INCLUDE_FOUND "${FIRST_ARMPL_H}" DIRECTORY)
    get_filename_component(FOUND_ARMPL_DIR "${ARMPL_INCLUDE_FOUND}" DIRECTORY)
...
# ============================================================================
# CRITICAL FIX: Use SHARED IMPORTED for Windows DLLs
# ============================================================================
if(NOT TARGET armpl)
    if(WIN32)
        # Windows: Use SHARED IMPORTED type for DLLs
        # IMPORTED_IMPLIB = .lib file (used by linker)
        # IMPORTED_LOCATION = .dll file (used at runtime)
        ...
        # Unix: Use UNKNOWN IMPORTED type
...
set(ARMPL_OPENMP_ENABLED FALSE)
if(ARMPL_LIB_NAME MATCHES "_mp$")
    set(ARMPL_NEEDS_OPENMP TRUE)
    
    if(NOT WITH_OPENMP)
        set(WITH_OPENMP ON CACHE BOOL "Include OpenMP support (auto-enabled for ArmPL)" FORCE)
    endif()
    
    find_package(OpenMP QUIET)
    if(OpenMP_CXX_FOUND)
        set(ARMPL_OPENMP_ENABLED TRUE)
        set_target_properties(armpl PROPERTIES
            INTERFACE_LINK_LIBRARIES OpenMP::OpenMP_CXX
        )
        
        if(WIN32)
            set(LIBOMP_LIB_PATH "${ARMPL_LIBRARY_DIR}/libomp.dll.lib")
            if(EXISTS "${LIBOMP_LIB_PATH}")
                set_target_properties(armpl PROPERTIES
                    INTERFACE_LINK_LIBRARIES "OpenMP::OpenMP_CXX;${LIBOMP_LIB_PATH}"
                )
                
                file(TO_NATIVE_PATH "${ARMPL_LIBRARY_DIR}" ARMPL_LIB_DIR_NATIVE)
                set(ARMPL_LINKER_FLAGS "/LIBPATH:\"${ARMPL_LIB_DIR_NATIVE}\" libomp.dll.lib")
