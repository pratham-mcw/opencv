...
static void transpose8x8_u8(
    uint8x8_t& r0, uint8x8_t& r1, uint8x8_t& r2, uint8x8_t& r3,
...
static void transpose_16SC3_neon(const uchar* src, size_t sstep, uchar* dst, size_t dstep, Size sz)
{
    ...
    for (; i <= m - 8; i += 8)
    {
        ...
        for (; j <= n - 8; j += 8)
        {
            ...
            uint16x8x3_t q0; q0.val[0]=R0; q0.val[1]=G0; q0.val[2]=B0; vst3q_u16(reinterpret_cast<uint16_t*>(dst + dstep*(i+0) + j*6), q0);
            uint16x8x3_t q1; q1.val[0]=R1; q1.val[1]=G1; q1.val[2]=B1; vst3q_u16(reinterpret_cast<uint16_t*>(dst + dstep*(i+1) + j*6), q1);
            uint16x8x3_t q2; q2.val[0]=R2; q2.val[1]=G2; q2.val[2]=B2; vst3q_u16(reinterpret_cast<uint16_t*>(dst + dstep*(i+2) + j*6), q2);
            uint16x8x3_t q3; q3.val[0]=R3; q3.val[1]=G3; q3.val[2]=B3; vst3q_u16(reinterpret_cast<uint16_t*>(dst + dstep*(i+3) + j*6), q3);
            uint16x8x3_t q4; q4.val[0]=R4; q4.val[1]=G4; q4.val[2]=B4; vst3q_u16(reinterpret_cast<uint16_t*>(dst + dstep*(i+4) + j*6), q4);
            uint16x8x3_t q5; q5.val[0]=R5; q5.val[1]=G5; q5.val[2]=B5; vst3q_u16(reinterpret_cast<uint16_t*>(dst + dstep*(i+5) + j*6), q5);
            uint16x8x3_t q6; q6.val[0]=R6; q6.val[1]=G6; q6.val[2]=B6; vst3q_u16(reinterpret_cast<uint16_t*>(dst + dstep*(i+6) + j*6), q6);
            uint16x8x3_t q7; q7.val[0]=R7; q7.val[1]=G7; q7.val[2]=B7; vst3q_u16(reinterpret_cast<uint16_t*>(dst + dstep*(i+7) + j*6), q7);
        }
        for (; j < n; j++)
            for (int k = 0; k < 8; k++)
